...
**4.凭证代币机制**  
**业务需求**：用户存了钱不能马上取，得等结束后或者清算后按份额取，但是可以申领，只要状态不是MATCH或者UNDONE就能申领  
**代码实现思路**：定义claim函数，根据用户的存款数量和池子总存款数量计算份额，再用份额和实际借出/借入数量计算凭证代币数量，给用户铸造凭证代币以此提取资产，用户可以用claim函数申领。定义withdraw函数，先销毁凭证，然后计算该凭证对应的资产数量给用户转账。另外，对于借款人而言需要给借款人转实际借出的资产，可以在claim函数按份额索取  

**5.退款机制**  
**业务需求**：结算后多余的资产可以退回，只要状态不是MATCH或者UNDONE就能退  
**代码实现思路**：定义refund函数，先检查用户是否有存入、是否重复退款、池中是否有未退还资产，然后计算总退款数量*用户份额得到用户可退款数量，转给用户  

**6.紧急提取资产**  
**业务需求**：状态为UNDONE时允许用户紧急提取  
**代码实现思路**：定义emergencyWithdrawal函数，检查状态是否为UNDONE、用户是否有存入、是否重复提取，然后将用户存入的资产全部转回  
