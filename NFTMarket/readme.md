...
4、批量编辑订单  
传入旧订单标识oldOrderKey和newOrder，用oldOrderKey获取旧订单，进行编辑限制检查和新订单验证后，调用Storage合约的removeOrder和cancelOrder函数移除订单，然后调用Storage合约的addOrder函数添加newOrder，然后调用Vault合约的editNFT更新NFT关联或者editETH更新ETH关联，对于Bid订单，如果新价格更高需要补足差额ETH，如果新价格更低金库会退回多余ETH  

5、撮合单个订单  
传入sellOrder和buyOrder，验证sellOrder和buyOrder是否匹配，然后调用Validator合约的validateOrder函数验证taker是否在Storage中，按两种场景分别处理  
卖家接受出价：sellOrder.maker调用，验证buyOrder，要求Bid订单必须存在于订单存储中，此时成交价格为Bid订单的价格，然后调用Validator合约的updateFilledAmount函数更新成交数量，然后进行资产转移，调用Vault合约的withdrawETH函数从金库提取ETH到OrderBook合约，扣除手续费后转给卖家，NFT则调用Vault合约的withdrawNFT函数从卖家转移到买家，或者sellOrder不存在于订单存储中则直接调用Vault合约的transferERC721函数直接转给买家  
买家接受挂单：buyOrder.maker调用，验证sellOrder，要求List订单必须存在于订单存储中，此时成交价格为List订单的价格，然后Validator合约的updateFilledAmount函数更新成交数量，然后进行资产转移，调用Vault合约的withdrawETH函数从金库提取ETH到OrderBook合约，扣除手续费后转给卖家，NFT则调用Vault合约的withdrawNFT函数从卖家转移到买家。如果买家出价高于成交价，退回多余ETH，如果买家传入的ETH高于实际花费的ETH金额，退回多余ETH  
